...
export default function Under400() {
    ...
    // Initialize with all products under 400
    useEffect(() => {
        const productsUnder400 = products.filter(product =>
            product.variant.some(v => (v.discountPrice || v.price) <= 400)
        );
        setFilteredProducts(productsUnder400);
    }, []);

    // Apply filters - modified to only show products under 400
    useEffect(() => {
    ...
    // Only show colors that exist in products under 400
    const allColors = [...new Set(
        products
            .filter(product => product.variant.some(v => (v.discountPrice || v.price) <= 400))
            .flatMap(product => product.variant.map(v => v.color))
    )];


    // Only show categories that have products under 400
    ...
    return (
        <div className="bg-gray-50 min-h-screen">
            ...
            <div className="w-full">
                <img
                    src="/banner/under-400.webp"
                    alt="Products Under ₹400"
                    className="w-full object-cover"
                />
            </div>

            <div className='text-center mt-5'>
                <h1 className="text-2xl md:text-4xl font-bold mb-3">All Products Under ₹400</h1>
            </div>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                                            ...
                                            <div className="mt-3 space-y-2">
                                                {allColors.map(color => {
                                                    const colorCount = products.filter(product =>
                                                        product.variant.some(v => v.color === color && (v.discountPrice || v.price) <= 400)
                                                    ).length;
                    ...
                    <div className="w-full lg:w-3/4">
                        ...
                        <div className="hidden lg:flex justify-between items-center mb-4">
                            <span className="text-gray-600">{filteredProducts.length} products under ₹400</span>
                        ...
                        {filteredProducts.length === 0 ? (
                            <div className="text-center sm:py-16 py-6 bg-white rounded-xl shadow-sm border border-gray-100">
                                <h3 className="text-xl font-medium text-gray-700 mb-4">No products under ₹400 match your filters</h3>
                                <button
                                    onClick={resetFilters}
                                ...
                                {filteredProducts.map(product => {
                                    ...
                                    return (
                                        <div key={product._id} className="group relative">
                                            <div className="bg-white rounded-md overflow-hidden shadow-sm hover:shadow-md transition duration-300 h-full flex flex-col border border-[#7a1113]">
