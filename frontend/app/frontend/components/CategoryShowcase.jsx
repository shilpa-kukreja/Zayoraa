...
const CategoryShowcase = () => {
  ...
  // ✅ Sort categories by 'order' (ascending, lower number first)
  const sortedCategories = useMemo(() => {
    return [...categories].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  }, [categories]);

  // ✅ Set default category to the first sorted category
  useEffect(() => {
    if (sortedCategories.length > 0 && !activeCategoryId) {
      setActiveCategoryId(sortedCategories[0]._id);
    }
  ...
  return (
    <section className="max-w-[1600px] mx-auto px-2 sm:px-6 lg:px-8 sm:py-16 py-6">
      ...
      {/* Category Tabs - now using sortedCategories */}
      <div className="flex justify-center flex-wrap gap-4 sm:mb-10 mb-6">
