...
export const AppContextProvider = ({ children }) => {
  ...
  const syncCartToBackend = useCallback(() => {
    ...
    syncTimer.current = setTimeout(() => {
      axios
        .post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/cart/sync`, {
          userId,
          cartItems: items,
        })
        .then(() => console.log("✅ Cart synced"))
        .catch((err) => console.error("Sync failed:", err));
    }, 2000);
  }, [userId, cartItems]);

  // Trigger sync whenever cartItems or userId change
  useEffect(() => {
    syncCartToBackend();
    return () => {
      if (syncTimer.current) clearTimeout(syncTimer.current);
    };
  }, [cartItems, syncCartToBackend]);

  // ===================== RESTORE HELPER (used in restore page) =====================
  const restoreCartFromBackend = async (token) => {
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/cart/restore?token=${token}`
    );
...
//   setIsCartOpen(true);
  ...
  setCartItems(updatedCart);
    ...
    const value = {
        ...
        isInWishlist,
         userId,                 // <-- NEW
    restoreCartFromBackend, // <-- NEW
    syncCartToBackend,      // <-- NEW (optional)
    };
